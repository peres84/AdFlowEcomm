...
project_root = os.path.dirname(script_dir)  # Go up one level from zdemo/ to project root
...
import base64
from zdemo.demo_campaign_config import CampaignConfig, BrandTone, TargetPlatform, SceneVibeDescription, get_mockup_config
...
def analyze_product_image_detailed(image_path: str) -> dict:
    """
    Analyze product image using OpenAI Vision API with detailed product information.
    This is the same function from demo_dynamic_campaign.py
    
    Args:
        image_path: Path to product image file or URL
        
    Returns:
        Dictionary with product_type, description, colors, materials, style
    """
    api_key = os.getenv("OPENAI_API_KEY")
    if not api_key:
        print("⚠️  Warning: OPENAI_API_KEY not set, skipping image analysis")
        return {}
        ...
        prompt = """Analyze this product image and provide a JSON response with:
{
  ...
  "style": "visual style"
        ...
        try:
            ...
            print("⚠️  Could not parse JSON from analysis")
...
def generate_scene_prompt_dynamic(scene_type: str, config: CampaignConfig) -> str:
    """
    ...
    Returns:
        Formatted prompt string for the scene
    """
    ...
    if scene_type == "hook":
        ...
        Product to showcase: {config.product_description if config.product_description else config.product_name}
        
        IMPORTANT: Focus exclusively on THIS specific product - the {config.product_type if config.product_type else config.product_category}.
        ...
        Commercial quality ready for {config.target_platform.value if config.target_platform else 'social media'}.
        ...
        The {config.product_type if config.product_type else config.product_name} works autonomously in the background or foreground.
        
        Product details: {config.product_description if config.product_description else config.product_name}
        Setting: {environment}
        ...
        Authentic {config.brand_tone.value.lower() if config.brand_tone else 'professional'} tone with genuine emotions.
        The product should be clearly visible.
        Professional lifestyle photography quality for {config.target_platform.value if config.target_platform else 'social media'}."""
        
    else:  # cta
        prompt = f"""Hero product shot featuring ONLY the {config.product_name} for call-to-action.
        
        PRODUCT FOCUS:
        Show exclusively THIS product: {config.product_description if config.product_description else config.product_name}
        ...
        Premium {config.brand_tone.value.lower() if config.brand_tone else 'professional'} aesthetic.
        Perfect for final frame of {config.target_platform.value if config.target_platform else 'social media'} ad - drives viewers to take action.
        Commercial quality, marketing-ready, professional product photography."""
...
Beispiel:
  python zdemo/demo_main.py \\
    ...
    parser.add_argument(
        "--use-campaign-config",
        dest="use_campaign_config",
        action="store_true",
        help="Use CampaignConfig structure instead of simple theme/vibe/details. Requires --campaign-style parameter."
    )
    ...
    parser.add_argument(
        ...
        dest="campaign_style",
        type=str,
        choices=["luxury", "lifestyle", "energetic"],
        default="luxury",
        help="Campaign style when using --use-campaign-config (default: luxury)"
    )
    ...
    try:
        ...
        # Check if using CampaignConfig mode
        campaign_config = None
        if args.use_campaign_config:
            print("🔄 Using CampaignConfig structure...")
            campaign_config = get_mockup_config(args.campaign_style)
            print(f"✅ Campaign config loaded: {campaign_config.product_name}")
            print(f"   🎨 Brand Tone: {campaign_config.brand_tone.value}")
            print(f"   📱 Platform: {campaign_config.target_platform.value}")
            print()
        
        # Schritt 1a: Präzisiere User-Input (Theme, Vibe, Details) mit ChatGPT
        if args.use_campaign_config:
            # Skip refinement if using CampaignConfig
            refined_context = {
                "theme": campaign_config.product_category,
                "vibe": campaign_config.scene_vibe.to_prompt_context() if campaign_config.scene_vibe else None,
                "details": f"{campaign_config.target_audience}, {campaign_config.main_benefit}"
            }
        elif args.theme or args.vibe or args.details:
        ...
        # First, do detailed product image analysis (from demo_dynamic_campaign.py)
        detailed_analysis = analyze_product_image_detailed(args.product_image_url)
        if detailed_analysis:
            print(f"✅ Detailed product analysis:")
            if "product_type" in detailed_analysis:
                print(f"   🏷️  Product Type: {detailed_analysis.get('product_type', 'N/A')}")
            if "colors" in detailed_analysis:
                print(f"   🎨 Colors: {', '.join(detailed_analysis.get('colors', []))}")
            if "materials" in detailed_analysis:
                print(f"   🔧 Materials: {', '.join(detailed_analysis.get('materials', []))}")
            print()
        ...
        # Merge detailed analysis into product_data if available
        if detailed_analysis:
            if "product_type" in detailed_analysis and not product_data.get("product_type"):
                product_data["product_type"] = detailed_analysis.get("product_type")
            if "description" in detailed_analysis and not product_data.get("description"):
                product_data["description"] = detailed_analysis.get("description")
            if "colors" in detailed_analysis:
                product_data["colors"] = detailed_analysis.get("colors", [])
            if "materials" in detailed_analysis:
                product_data["materials"] = detailed_analysis.get("materials", [])
